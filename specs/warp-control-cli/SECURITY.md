...
- Keep the action surface at exactly 90 allowlisted actions. The Block, Auth, Drive, and History families are entirely absent.
...
- Connect to the broker socket and request credentials for any of the 90 actions.
...
4. Broker checks Scripting is enabled and the action is in the 90-action catalog.
...
The two input commands (`input.insert`, `input.replace`) only stage text in the terminal input buffer. They never submit the buffer, press Enter, or execute a command. No other input actions (`input.get`, `input.clear`, `input.mode.set`, `input.run`) exist in the 90-action catalog. Tests must prove no submission occurs.
## Catalog boundary
The catalog contains exactly 90 actions. The following families and actions are entirely absent:
...
- The action has a documented entry in the 90-action catalog.
...
- The catalog contains exactly 90 default-authorized actions.
