...
16. Chips appear in all three Vertical Tabs item modes that show the metadata line:
    ...
    - Summary: on the tab's summary card, as the union of the links from all the tab's panes with duplicates by label removed (first occurrence wins, in pane order). Links are placed on the branch line of the pane's repository when one exists; when the tab has no branch lines at all but has links, Summary draws one metadata line that contains only the link chips.
